...
public class WarehouseClotPrint implements Printable {

	/** 序号 */
	private String code = "14843";
	/** 批次 */
	private String clot = "80340G211L";
	/** 仓库 */
	private String cwar = "CRB02";
	/** 物料 */
	private String item = "12313212";
	/** 物料说明 */
	private String dsca = "(B)硅铁Si≥72%,C≤0.2% P≤0.05% Cr≤0.3% S≤0.02% Al≤1.8% Mn≤0.4% 粒S≤0.02% Al≤1.8% Mn≤0.4% 粒";
	/** 物料说明 */
	private String user = "N051324";
	/** 左边距 */
	...
	public WarehouseClotPrint(String printerName) {
		super();
		this.printerName = printerName;
	}

	public WarehouseClotPrint() {
		super();
	}
...
//				 打印起点坐标
		double x = pf.getImageableX() + 10;
		double y = pf.getImageableY();
		switch (pageIndex) {
		case 0: 
			Font font = new Font("Arial", Font.PLAIN, 10);
			g2.setFont(font);
			g2.drawString("批号", (float) (x + 5), (float) (y + 25));
			font = new Font("Arial", Font.PLAIN, 8);
			g2.setFont(font);
			g2.drawString( bpidname.trim(), (float) (x + 5), (float) (y + 45));
			g2.drawString("物料：" + item.trim(), (float) (x + 5), (float) (y + 55));
			g2.drawString(user.trim(), (float) (x + 110), (float) (y + 55)); 
			if (dsca.length() > 50) {
				g2.drawString(dsca.substring(0, 50), (float) (x + 5), (float) (y + 65));
				if (dsca.length() > 100)
					{g2.drawString(dsca.substring(51,100), (float) (x + 5), (float) (y + 75));}
				else
					{g2.drawString(dsca.substring(51,  dsca.length() ), (float) (x + 5), (float) (y + 75));}
			} else {
				g2.drawString(dsca, (float) (x + 5), (float) (y + 65));
			}
			CodeAndPrint codeAndPrint = new CodeAndPrint();
			String filename = "C:\\toolsZ\\production\\codeZ\\11.jpg";
			codeAndPrint.printDimensionalCode(clot, filename);
			Image Img = null;
			try {
			...
			g2.drawImage(Img, (int) x + 25, (int) y + 5, 180, 30, c);
			return PAGE_EXISTS;
		default:
			return NO_SUCH_PAGE;
		}
	...
	public static void main(String[] args) {
		...
		Paper p = new Paper();
		p.setSize(1062, 354);// 纸张大小
		// A4(595 X 842)设置打印区域，其实0，0应该是72，72，因为A4纸的默认X,Y边距是72
		p.setImageableArea(0, 0, 1062, 354);
		pf.setPaper(p);
		// 把 PageFormat 和 Printable 添加到书中，组成一个页面
		book.append(new WarehouseClotPrint(), pf);
	...
	public void printcode() {
		...
		paper.setSize(1062, 354);// 纸张
		// A4(595 X 842)设置打印区域，其实0，0应该是72，72，因为A4纸的默认X,Y边距是72
		paper.setImageableArea(0, 0, 1062, 354);
